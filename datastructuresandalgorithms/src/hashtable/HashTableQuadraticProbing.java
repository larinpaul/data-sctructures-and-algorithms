 ...
 * quadratic probing as a collision reduction method
 * 
 * In this implementation we are using the following probing functions:
 * 			H(k, x) = h(k) + f(x) mod 2^n
 * 
 * Where h(k) is the hash for the given key, f(x) = (x + x^2) / 2 and n is a natural number.
 * We are using this probing function because it
...
public class HashTableQuadraticProbing <K, V> implements Iterable <K> {
	...
	// Return true/false on whether a given key exists within the hash-table
	...
	public V get(K key) {
		...
		do {
			...
			// of a deleted cell is found to performs lazy relocation later.
			if (keyTable[i] == TOMBSTONE) {
			...
			// We hit a non-null key, perhaps it's the one we're looking for.
				...
				if (keyTable[i].equals(key)) {
					...
					// We can perform an optimization by swapping the entires in cells
	...
	// Return a String version of this bash-table
