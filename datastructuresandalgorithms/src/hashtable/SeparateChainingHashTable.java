...
class Entry<K, V> {
	...
	// We are no overriding the Object equals method
...
public class SeparateChainingHashTable <K,V> implements Iterable <K> {
	...
	public V remove(K key) {
		...
		if (key == null) return null;
		int ucketIndex = normalizeIndex(key.hashCode());
	...
	// exists in the given bucket, but if it does then update the entry value
	private V bucketInsertEntry(int bucketIndex, Entry<K,V> entry) {
		...
		if (bucket == null) {
			...
			return null; // Use null to indicate that that there was no previous entry
		} else {
	...
	// Finds an returns a particular entry in a given bucket if it exists, returns null otherwise
	...
	// Resizes the internal table holding buckets of entires
